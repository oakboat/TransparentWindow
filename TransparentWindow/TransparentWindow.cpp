...
static IDCompositionDevice* g_pDCompDevice = nullptr;        // DirectComposition 设备
static IDCompositionTarget* g_pDCompTarget = nullptr;        // 合成目标
static IDCompositionVisual* g_pDCompVisual = nullptr;        // 合成视觉对象
...
int main(int, char**)
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowExW(WS_EX_NOREDIRECTIONBITMAP, wc.lpszClassName, L"Dear ImGui DirectX12 Example", WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, nullptr, nullptr, wc.hInstance, nullptr);
...
bool CreateDeviceD3D(HWND hWnd)
{
...

    {
        ...
        if (CreateDXGIFactory2(0, IID_PPV_ARGS(&dxgiFactory)) != S_OK)
            return false;
        if (dxgiFactory->CreateSwapChainForComposition(g_pd3dCommandQueue, &sd, nullptr, &swapChain1) != S_OK)
            return false;
        if (DCompositionCreateDevice(nullptr, IID_PPV_ARGS(&g_pDCompDevice)) != S_OK)
            return false;
        if (g_pDCompDevice->CreateTargetForHwnd(hWnd, TRUE, &g_pDCompTarget) != S_OK)
            return false;
        if (g_pDCompDevice->CreateVisual(&g_pDCompVisual) != S_OK)
            return false;
        if (g_pDCompVisual->SetContent(swapChain1) != S_OK)
            return false;
        if (g_pDCompTarget->SetRoot(g_pDCompVisual) != S_OK)
            return false;
        if (g_pDCompDevice->Commit() != S_OK)
            return false;
